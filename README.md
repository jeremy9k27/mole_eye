...
Mole-Eye is inspired by Hawk-Eye Innovation's pitch classifier, for which they use 12 high speed cameras to observe the ball's movement, velocity, spin rate, and spin axis. Mole_Eye, a low-scale, accessible version of this, is a Python code that uses the CV2 library to continuously recieve frames from an iPhone camera and run algorithms to display and classify pitches in real time. The code detects the ball based on the pixels in each frame that are within a carefully picked color range - increasing the range allows more of the ball to be captured but also increases the amount of false-positives. The location of the ball is calculated as the centroid of each of the pixels within the color range, with pixels that are far away from the rest of the pixels in the range (likely false-positives) not used in the centroid calculation. Mole-Eye's algorithm for knowing if a pitch is actually in frame is based on whether centroids are sufficently close to each other for certain time range. If the centroids are instead jumping widely from frame to frame, these centroids are likely the result of false-positives, not the ball actually being in frame. Likewise, Mole-Eye detects that the pitch has ended once the centroids switch from being close together to jumping. Mole-Eye then displays each centroid from the pitch onto a singular frame and uses the centroids to classify the pitch into pitchtypes (fastball, slider, etc).
